...
namespace Common
{
    ...
    public static class Tools
    {
        ...
        /// <returns></returns>
        public static T CopyObjectValue<T>(T RtnObj, T OldObj)
        {
            var PurchaseOrderRf = OldObj.GetType();

            foreach (var Property in PurchaseOrderRf.GetProperties())
            {
                string Name = Property.Name;
                var property = PurchaseOrderRf.GetProperty(Name);

                if (property.GetCustomAttributes(typeof(KeyAttribute), true).Length <= 0)
                {    
                    object o = property.GetValue(OldObj);
                    object oRtnObj = property.GetValue(RtnObj);

                    if (string.IsNullOrEmpty(o?.ToString()))
                    {
                        continue;
                    }
                   property.SetValue(RtnObj, o);  
                }
